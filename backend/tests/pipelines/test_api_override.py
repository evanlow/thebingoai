...
is patched out. The endpoint handlers are `async def`, so each call goes
through `asyncio.run(...)` — pytest-asyncio is not installed in this image.
"""
...
def test_redetect_returns_classifier_suggestion(monkeypatch):
    ...
    result = _run(api.redetect_watermark("p1", _user(), db))
...
def test_redetect_503_when_schema_fetch_fails(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        _run(api.redetect_watermark("p1", _user(), db))
