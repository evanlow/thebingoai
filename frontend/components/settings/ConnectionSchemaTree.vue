<template>
  ...
  <div class="flex items-center gap-2 shrink-0">
    ...
    <span v-if="schema" class="text-sm text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded-full dark:bg-neutral-700 dark:text-neutral-400">
    ...
    <div class="overflow-y-auto flex-1 space-y-1">
      <div v-for="(schemaData, schemaName) in filteredSchemas" :key="schemaName">
        ...
        >
          ...
          <span class="text-sm font-medium text-gray-700 dark:text-neutral-200 truncate">{{ schemaName }}</span>
          <span class="text-sm text-gray-400 dark:text-neutral-500 ml-auto shrink-0">{{ Object.keys(schemaData.tables).length }}</span>
        </button>
        ...
        <div v-if="expandedSchemas[String(schemaName)]" class="ml-4 space-y-0.5">
          <div v-for="(tableData, tableName) in schemaData.tables" :key="tableName">
            ...
            >
              ...
              <span class="text-sm text-gray-700 dark:text-neutral-200 truncate">{{ tableName }}</span>
              <span v-if="tableData.row_count != null" class="text-sm text-gray-400 dark:text-neutral-500 ml-auto shrink-0">{{ tableData.row_count.toLocaleString() }}</span>
            </button>
            ...
            <div v-if="expandedTables[`${schemaName}.${tableName}`]" class="ml-6 space-y-0.5">
              <div
                ...
                :key="col.name"
                class="flex items-center gap-1.5 py-0.5 px-2 text-sm"
              >
                ...
                <span class="text-gray-600 dark:text-neutral-200 bg-gray-100 dark:bg-neutral-700 px-1 py-0.5 rounded font-mono ml-auto shrink-0 text-sm">{{ col.type }}</span>
