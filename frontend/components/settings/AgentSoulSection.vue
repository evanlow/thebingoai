  ...
  <div id="soul" class="border border-[var(--line)] rounded-[var(--r-lg)] p-6">
    ...
    <div class="border border-[var(--line)] rounded-[var(--r-md)] overflow-hidden mb-4">
      ...
      <div class="flex items-center justify-between px-3 py-1.5 bg-[var(--paper-1)] border-b border-[var(--line)]">
        <div class="flex gap-1 items-center">
          <span class="text-sm px-2 py-1 rounded-[var(--r-sm)] bg-[var(--paper-0)] border border-[var(--line)] text-[var(--ink-0)] font-medium font-mono">
            soul.md
          </span>
          <span class="text-sm px-2 py-1 text-[var(--ink-2)] font-mono">markdown</span>
        </div>
        <!-- Toolbar actions -->
        <div class="flex items-center gap-1">
          <button @click="showDiff = true"
                  :disabled="!hasPublishedDiff"
                  class="text-sm px-2 py-1 rounded-[var(--r-sm)] text-[var(--ink-2)] hover:bg-[var(--paper-2)] disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
            ⇄ Diff vs published
          </button>
          <button @click="onRevise"
                  :disabled="revising"
                  class="text-sm px-2 py-1 rounded-[var(--r-sm)] text-[var(--ember)] hover:bg-[var(--ember-wash)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors font-medium">
      ...
      <textarea
        ...
        rows="16"
        spellcheck="false"
        class="w-full p-4 font-mono text-sm leading-relaxed text-[var(--ink-1)] bg-[var(--paper-0)] resize-none outline-none border-none"
      ...
      <div class="flex items-center justify-between px-3 py-1.5 bg-[var(--paper-1)] border-t border-[var(--line)]">
        <span class="text-sm text-[var(--ink-3)] font-mono">
    ...
    <div class="mt-2">
      <div class="flex items-center justify-between mb-2">
        <p class="text-sm font-bold tracking-[.08em] uppercase text-[var(--ink-3)]">Style References</p>
        <button v-if="!addingRef" @click="startAddRef"
                class="text-sm font-semibold text-[var(--ember)] hover:opacity-80">
          + Add reference
        </button>
      </div>
      ...
      <div class="flex flex-wrap gap-2 mb-2">
        <div v-for="(ref, idx) in localRefs" :key="idx"
             class="inline-flex items-center gap-1.5 text-sm bg-[var(--paper-1)] border border-[var(--line)] text-[var(--ink-1)] px-2.5 py-1 rounded-[var(--r-sm)]">
        ...
        <div v-if="localRefs.length === 0 && !addingRef"
             class="text-sm text-[var(--ink-3)] italic">
      ...
      <div v-if="addingRef"
        ...
        <div class="grid grid-cols-[80px_1fr_1fr] gap-2 items-end mb-2">
          <div>
            <label class="block text-sm font-bold uppercase tracking-[.08em] text-[var(--ink-3)] mb-1">Type</label>
          ...
          <div>
            <label class="block text-sm font-bold uppercase tracking-[.08em] text-[var(--ink-3)] mb-1">Title</label>
            <input v-model="newRefTitle" ref="refTitleInputRef"
                   placeholder="e.g. Economist house style"
                   class="w-full text-sm border border-[var(--line)] rounded-[var(--r-sm)] px-2 py-1 bg-[var(--paper-0)] outline-none focus:border-[var(--ember)] text-[var(--ink-0)]" />
          </div>
          <div>
            <label class="block text-sm font-bold uppercase tracking-[.08em] text-[var(--ink-3)] mb-1">URL / ID</label>
        ...
        <div class="flex justify-end gap-2">
          <button @click="cancelAddRef"
                  class="text-sm font-medium border border-[var(--line)] text-[var(--ink-1)] px-2.5 py-1 rounded-[var(--r-sm)] bg-[var(--paper-0)] hover:bg-[var(--paper-2)] transition-colors">
            Cancel
          </button>
          <button @click="commitAddRef"
                  :disabled="!newRefTitle.trim() || !newRefPointer.trim()"
                  class="text-sm font-semibold text-white bg-[var(--ember)] px-2.5 py-1 rounded-[var(--r-sm)] hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-opacity">
            Add
          </button>
        </div>
      </div>
