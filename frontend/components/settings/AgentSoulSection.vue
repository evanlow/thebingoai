  ...
  <div id="soul" class="border border-[var(--line)] rounded-[var(--r-lg)] p-6">
    ...
    <div class="flex items-start justify-between mb-5">
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 rounded-[var(--r-sm)] bg-[var(--ember-wash)] border border-[color-mix(in_oklch,var(--ember)_20%,var(--line))] flex items-center justify-center text-base flex-shrink-0 text-[var(--ink-0)]">
          ✦
        </div>
        <div>
      ...
      <span class="text-xs font-semibold text-[var(--ink-2)] bg-[var(--paper-2)] border border-[var(--line)] px-2.5 py-1 rounded-full whitespace-nowrap">
    ...
    <div class="border border-[var(--line)] rounded-[var(--r-md)] overflow-hidden mb-4">
      ...
      <div class="flex items-center justify-between px-3 py-1.5 bg-[var(--paper-1)] border-b border-[var(--line)]">
        <div class="flex gap-1 items-center">
          <span class="text-xs px-2 py-1 rounded-[var(--r-sm)] bg-[var(--paper-0)] border border-[var(--line)] text-[var(--ink-0)] font-medium font-mono">
            soul.md
          </span>
          <span class="text-xs px-2 py-1 text-[var(--ink-2)] font-mono">markdown</span>
        </div>
        <!-- Toolbar actions -->
        <div class="flex items-center gap-1">
          <button @click="showDiff = true"
                  :disabled="!hasPublishedDiff"
                  class="text-xs px-2 py-1 rounded-[var(--r-sm)] text-[var(--ink-2)] hover:bg-[var(--paper-2)] disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
            ⇄ Diff vs published
          </button>
          <button @click="onRevise"
                  :disabled="revising"
                  class="text-xs px-2 py-1 rounded-[var(--r-sm)] text-[var(--ember)] hover:bg-[var(--ember-wash)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors font-medium">
      ...
      <div class="flex items-center justify-between px-3 py-1.5 bg-[var(--paper-1)] border-t border-[var(--line)]">
        <span class="text-xs text-[var(--ink-3)] font-mono">
    ...
    <div class="mt-2">
      <div class="flex items-center justify-between mb-2">
        <p class="text-xs font-bold tracking-[.08em] uppercase text-[var(--ink-3)]">Style References</p>
        <button v-if="!addingRef" @click="startAddRef"
                class="text-xs font-semibold text-[var(--ember)] hover:opacity-80">
          + Add reference
        </button>
      </div>
      ...
      <div class="flex flex-wrap gap-2 mb-2">
        <div v-for="(ref, idx) in localRefs" :key="idx"
             class="inline-flex items-center gap-1.5 text-xs bg-[var(--paper-1)] border border-[var(--line)] text-[var(--ink-1)] px-2.5 py-1 rounded-[var(--r-sm)]">
          <span class="text-sm">{{ ref.type === 'doc' ? '📄' : '🔗' }}</span>
          <span>{{ ref.title }}</span>
          <button @click="removeRef(idx)" class="text-[var(--line-2)] hover:text-[var(--ink-2)] ml-0.5">✕</button>
        </div>
        <div v-if="localRefs.length === 0 && !addingRef"
             class="text-xs text-[var(--ink-3)] italic">
      ...
      <div v-if="addingRef"
        ...
        <div class="grid grid-cols-[80px_1fr_1fr] gap-2 items-end mb-2">
          <div>
            <label class="block text-xs font-bold uppercase tracking-[.08em] text-[var(--ink-3)] mb-1">Type</label>
            <select v-model="newRefType"
                    class="w-full text-sm border border-[var(--line)] rounded-[var(--r-sm)] px-2 py-1 bg-[var(--paper-0)] outline-none focus:border-[var(--ember)] text-[var(--ink-0)]">
              <option value="link">Link</option>
              <option value="doc">Doc</option>
            </select>
          </div>
          <div>
            <label class="block text-xs font-bold uppercase tracking-[.08em] text-[var(--ink-3)] mb-1">Title</label>
            <input v-model="newRefTitle" ref="refTitleInputRef"
                   placeholder="e.g. Economist house style"
                   class="w-full text-sm border border-[var(--line)] rounded-[var(--r-sm)] px-2 py-1 bg-[var(--paper-0)] outline-none focus:border-[var(--ember)] text-[var(--ink-0)]" />
          </div>
          <div>
            <label class="block text-xs font-bold uppercase tracking-[.08em] text-[var(--ink-3)] mb-1">URL / ID</label>
        ...
        <div class="flex justify-end gap-2">
          <button @click="cancelAddRef"
                  class="text-xs font-medium border border-[var(--line)] text-[var(--ink-1)] px-2.5 py-1 rounded-[var(--r-sm)] bg-[var(--paper-0)] hover:bg-[var(--paper-2)] transition-colors">
            Cancel
          </button>
          <button @click="commitAddRef"
                  :disabled="!newRefTitle.trim() || !newRefPointer.trim()"
                  class="text-xs font-semibold text-white bg-[var(--ember)] px-2.5 py-1 rounded-[var(--r-sm)] hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-opacity">
            Add
          </button>
        </div>
      </div>
