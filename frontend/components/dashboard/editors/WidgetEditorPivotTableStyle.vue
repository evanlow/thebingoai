<template>
  <div class="h-full overflow-y-auto p-5 space-y-5 [&>*+*]:border-t [&>*+*]:border-gray-200 [&>*+*]:pt-5">
    ...
    <div class="space-y-2">
      ...
      <div v-if="localShowTitle" class="space-y-1">
        ...
        <input v-model="localTitle" type="text" placeholder="Pivot title…" :readonly="!editMode"
          ...
          :class="!editMode ? 'cursor-default bg-gray-50' : ''" @input="emitUpdate()" />
      </div>
    </div>
    ...
    <!-- Totals labels -->
    <div class="space-y-2">
      <h3 class="text-[11px] font-medium text-gray-500 uppercase tracking-wide">Totals Labels</h3>
      <div class="space-y-1">
        <label class="text-xs text-gray-700">Subtotal label</label>
        <input v-model="localSubtotalLabel" type="text" placeholder="Subtotal" :readonly="!editMode"
          class="w-full rounded border border-gray-200 bg-white px-2 py-1.5 text-xs disabled:bg-gray-50" @input="emitUpdate()" />
      </div>
      <div class="space-y-1">
        <label class="text-xs text-gray-700">Grand total label</label>
    ...
    <div class="space-y-1.5">
      ...
      <div class="flex rounded border border-gray-200 overflow-hidden">
        <button v-for="opt in missingOptions" :key="opt.value" type="button" :disabled="!editMode"
          ...
          :class="localMissingData === opt.value ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'"
      ...
      <div class="space-y-1.5">
        ...
        <div class="flex rounded border border-gray-200 overflow-hidden">
          <button v-for="opt in borderStyleOptions" :key="opt.value" type="button" :disabled="!editMode"
            class="flex-1 py-1.5 text-[11px] font-medium transition-colors border-r border-gray-200 last:border-r-0 disabled:opacity-40"
            :class="(localBorderStyle ?? 'solid') === opt.value ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'"
    ...
    <div class="space-y-3">
      ...
      <div class="flex items-center justify-between gap-2">
        ...
        <select v-model="localFontFamily" :disabled="!editMode"
          class="rounded border border-gray-200 bg-white px-2 py-1 text-xs disabled:bg-gray-50" @change="emitUpdate()">
      ...
      <div class="space-y-1.5">
        ...
        <div class="flex rounded border border-gray-200 overflow-hidden">
          <button v-for="opt in fontSizeOptions" :key="opt.value" type="button" :disabled="!editMode"
            class="flex-1 py-1.5 text-[11px] font-medium transition-colors border-r border-gray-200 last:border-r-0 disabled:opacity-40"
            :class="(localFontSize ?? 'sm') === opt.value ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'"
