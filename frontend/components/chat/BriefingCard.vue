<template>
  <div class="rounded-xl border border-[var(--line)] bg-[var(--paper-0)] overflow-hidden text-left">
    ...
    <!-- Loading -->
    <div v-if="loading" class="flex items-center gap-2 p-4 text-sm text-[var(--ink-2)]">
      <Loader2 class="h-4 w-4 animate-spin" />
      <span>Loading briefing…</span>
    </div>

    <!-- Generating -->
    <div v-else-if="briefing?.status === 'generating'" class="flex items-center gap-2 p-4 text-sm text-[var(--ink-2)]">
      <Loader2 class="h-4 w-4 animate-spin text-violet-500" />
      <span>Briefing generating…</span>
    </div>

    <!-- Failed -->
    <div v-else-if="briefing?.status === 'failed'" class="p-4 text-sm text-red-500">
    ...
    <template v-else-if="briefing?.status === 'ready' && briefing.payload">
      ...
      <div class="px-5 pt-4 pb-3">
        <p class="text-sm font-semibold tracking-widest uppercase text-[var(--ink-2)] mb-1.5">
        ...
        <p class="mt-1 text-sm text-[var(--ink-2)] leading-relaxed">
      ...
      <div v-if="briefing.payload.kpis.length" class="px-4 py-3 flex gap-2.5">
        ...
        >
          <div class="text-sm font-semibold tracking-widest uppercase text-[var(--ink-2)] truncate">{{ kpi.label }}</div>
          <div class="text-[15px] font-bold text-[var(--ink-0)] mt-0.5 tabular-nums">{{ kpi.value }}</div>
          <div
            v-if="kpi.delta_vs_prev"
            class="text-sm font-medium mt-0.5 tabular-nums"
      ...
      <div v-if="briefing.payload.sections.length" class="px-5 py-3">
        <p class="text-sm font-semibold text-[var(--ink-0)]">
          {{ stripLeadingNumber(briefing.payload.sections[0].heading) }}
        </p>
        <p class="mt-1 text-sm text-[var(--ink-2)] leading-relaxed line-clamp-3">
          {{ briefing.payload.sections[0].prose }}
        </p>
        <button
          v-if="briefing.payload.sections.length > 1"
          class="mt-2 text-sm text-[var(--ink-2)] hover:text-[var(--ink-0)] transition-colors"
      ...
      <div class="px-5 py-3 flex items-center justify-between gap-3">
        <span class="text-sm text-[var(--ink-2)]">
        ...
        <button
          class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[var(--ink-0)] text-[var(--paper-0)] text-sm font-medium hover:opacity-80 transition-opacity flex-shrink-0"
          @click="open(briefing!.id)"
        >
