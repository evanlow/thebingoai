    ...
    <div :class="node.type === 'reasoning' ? 'items-start' : 'items-center'" class="flex text-sm py-0.5 min-h-[18px]">
      ...
      <div class="flex items-center gap-1.5 flex-1 min-w-0">
        ...
        <span
          ...
          :class="node.status === 'streaming' ? 'bg-amber-400 animate-pulse' : 'bg-gray-300 dark:bg-neutral-600'"
          class="w-1.5 h-1.5 rounded-full shrink-0"
        />
        ...
        <span
          :class="[
            node.type === 'agent' ? 'font-medium text-gray-800 dark:text-neutral-200' :
            node.type === 'reasoning' ? 'italic text-gray-600 dark:text-neutral-400 whitespace-normal break-words' :
            'truncate text-gray-600 dark:text-neutral-400'
          ]"
        ...
        <span
          ...
          class="text-sm bg-gray-200/70 dark:bg-neutral-700/70 text-gray-500 dark:text-neutral-400 px-1 py-px rounded-full ml-1 shrink-0"
...
.prose-detail .prose-chat {
  @apply text-sm text-gray-500 dark:text-neutral-400 leading-tight;
}
.prose-detail .prose-chat h1,
.prose-detail .prose-chat h2,
.prose-detail .prose-chat h3,
.prose-detail .prose-chat h4 {
  @apply text-sm font-medium mt-1 mb-0.5 text-gray-500 dark:text-neutral-400;
}
